...
int tab1[COO][COO], tab2[COO][COO], navi1=FLOTTA, navi2=FLOTTA;
...
void Inserimento1()
{
	...
	for(i=0; i<FLOTTA; i++)
	{
		do
		{
			...
			scanf("%d%d",&x,&x1);
			if(x<0 || x1<0 || x>=COO || x1>=COO || tab1[x][x1]==1)
			{
				occupato=true;
				system("color 0c");
...
void Inserimento2()
{
	...
	for(i=0; i<FLOTTA; i++)
	{
		do
		{
			printf("\n Inserisci la Nave n. %d \t",i+1);
			scanf("%d%d",&x,&x1);
			if(x<0 || x1<0 || x>=COO || x1>=COO || tab2[x][x1]==1)
			{
				occupato=true;
				system("color 0c");
				printf("\n Errore, coordinate non valide :( \n\n");
